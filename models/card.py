...
class CardSchema(ma.Schema):
    ...
    title = fields.String(required=True, validate=And(Length(min=2, error="Title must be atleast 2 characters long"), Regexp('^[a-zA-Z0-9 ]+$', error="Title can only have alphanumeric characters")))

    status = fields.String(validate=OneOf(VALID_STATUSES))

    priority = fields.String(validate=OneOf(VALID_PRIORITIES))

    # The can only be one card with status 'Ongoing'
    @validates('status')
    def validate_status(self, value):
        if value == VALID_STATUSES[1]:
            ...
            if count > 0:
                raise ValidationError('You allready have an ongoing card')
